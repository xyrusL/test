...
class FetchAnimeModel extends CI_Model
{
    ...
    {
        try {
            $this->db->truncate('featuredpost');
            
            // Add position field to each record based on array order
            foreach ($data as $index => $item) {
                $data[$index]['position'] = $index + 1;
            }
            
            $query = $this->db->insert_batch('featuredpost', $data);
            return $query;
        } catch (Exception $e) {
    ...
    {
        $this->db->order_by('position', 'ASC');  // Changed from 'id' to 'position'
        $query = $this->db->get('featuredpost');
